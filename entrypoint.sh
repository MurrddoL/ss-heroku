...
cat <<-EOF > /etc/shadowsocks-libev/config.json
{
    ...
    "no_delay":true,
    "plugin": "v2ray-plugin",
    "plugin_opts":"server;mux=0;loglevel=none;path=${V2_Path}"
}
EOF
...
if [ "$AppName" = "no" ]; then
  ...
  ss="ss://$(echo -n ${ENCRYPT}:${PASSWORD} | base64 -w 0)@${AppName}.herokuapp.com:14521?plugin=${plugin}" 
